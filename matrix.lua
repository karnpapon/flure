...
function matrix.det(m1)
  ...
  if size == 3 then
    return (m1[1][1] * m1[2][2] * m1[3][3] + m1[1][2] * m1[2][3] * m1[3][1] +
  ...
  for n = 1, iters do
    ...
    mx, my = mx:mulnum(root - 1):add(my:invert()):divnum(root),
             my:mulnum(root - 1):add(mx:invert()):divnum(root)
                 ...
                 :mul(my:mulnum(root - 1):add(mx:invert())):divnum(root)
...
function matrix.random(mtx, start, stop, idp)
  ...
  for i = 1, #mtx do
    for j = 1, #mtx[1] do mtx[i][j] = ffill(mtx[i][j], start, stop, idp) end
  end
  return mtx
end
...
function matrix.tostring(mtx, formatstr)
  ...
  local e = mtx[1][1]
  local tostring =
      mtype == "tensor" and tensor_tostring or type(e) == "table" and e.tostring or
          number_tostring
  for i = 1, #mtx do
    local tstr = {}
...
matrix_meta.__eq = function(m1, m2)
  ...
  for i = 1, #m1 do
    for j = 1, #m1[1] do if m1[i][j] ~= m2[i][j] then return false end end
  end
  return true
end
